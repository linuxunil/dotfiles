...
### Added
- **`oil.nvim`**: Added `oil.nvim` as the primary file explorer, replacing the `snacks.nvim` explorer.

### Changed
- **Replaced `snacks.nvim` explorer**: The `snacks.nvim` explorer has been replaced with `oil.nvim` to resolve startup errors and provide a more stable file management experience.
- **Keybinding Consolidation**: Resolved several keybinding conflicts and redundancies.
- **Autocommand Cleanup**: Removed several redundant and problematic autocommands.

### Removed
- **`zen-mode.nvim`**: Removed in favor of the `snacks.nvim` zen mode.
- **`indent-blankline.nvim`**: Removed in favor of the `snacks.nvim` indent guide module.
- **`project.nvim`**: Removed in favor of the `snacks.nvim` project picker.
